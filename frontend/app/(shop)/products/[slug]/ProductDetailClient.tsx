...
import { ChevronLeft, ChevronRight, Play, ShoppingBag, ChevronDown, ArrowRight, Zap, Heart, Check, Share2, Link as LinkIcon, MessageCircle, Facebook, Twitter } from 'lucide-react'
import {
  Product,
  ProductMedia,
  ProductVariant,
...
export function ProductDetailClient({ product, displayName, bodyCopy }: ProductDetailClientProps) {
  ...
  function getShareUrl(): string {
    const fallback = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.vamiclubwear.in'
          ...
          <div className="flex flex-col">
                    ...
                    <AnimatePresence>
                      {shareOpen && (
                        <motion.div
                          ...
                          role="menu"
                          className="absolute right-0 top-12 z-30 w-48 overflow-hidden rounded-md border border-border bg-background shadow-xl"
                        >
                          <a
                            ...
                            className="flex items-center gap-2 px-3 py-2.5 text-xs text-on-background hover:bg-surface-elevated"
                            role="menuitem"
                          >
                          ...
                          <button
                            ...
                            className="flex w-full items-center gap-2 border-t border-border px-3 py-2.5 text-xs text-on-background hover:bg-surface-elevated"
                            role="menuitem"
                          >
                            {linkCopied
            ...
            >
              ...
              <dl className="grid grid-cols-1 gap-y-2 text-sm sm:grid-cols-[140px_1fr]">
                ...
                {(() => {
                  // POS scan code for the current selection: the picked colour's
                  // code for per-colour products, otherwise the product barcode.
                  const code = product.perColorBarcode
                    ? (variant?.color
                        ? (product.colorBarcodes ?? []).find((c) => c.color === variant.color)?.barcode ?? null
                        : null)
                    : (product.barcode ?? null)
                  return code ? (
                    <>
                      <dt className="text-muted uppercase tracking-wider text-[11px]">Code</dt>
                      <dd className="font-mono text-on-background">{code}</dd>
                    </>
                  ) : null
                })()}
              </dl>
